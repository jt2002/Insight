...
def preprocess_review(review_file, review_pkl):
    ...
    try:
        review_df = pd.read_csv(
            review_file, dtype={"asin": object, "reviewerID": object}, sep=","
        )
    except:
    ...
    review_df.drop_duplicates(subset=["reviewerID", "asin"], keep="first", inplace=True)
...
def preprocess_product(product_file, product_pkl):
    ...
    try:
        chunk_df = pd.read_json(
            product_file, lines=True, dtype={"asin": object}, chunksize=chunksize
        )
    except:
    ...
    product_df.category = (
        product_df.category.str.replace(" >  >  > ", "")
        .str.replace(" >  > ", "")
        .str.replace(" > $", "")
    )
